...
class _PantallaReservasHState extends State<PantallaReservasH> {
  ...
  Future<void> _cerrarSesionAccion() async {
    try {
  ...
  List<Widget> _buildNavItems(BuildContext context, {bool isVertical = false}) {
    final double sizeFont = isVertical ? 18 : 25;
    return [
      TextButton.icon(
        onPressed: () {
        ...
        label: Text('Dashboard', style: TextStyle(color: const Color(0xFF216A44), fontSize: sizeFont)),
      ),
      TextButton.icon(
        onPressed: () {
        ...
        label: Text('Publicaciones', style: TextStyle(color: const Color(0xFF216A44), fontSize: sizeFont)),
      ),
      TextButton.icon(
        onPressed: null,
        icon: Icon(Icons.send_outlined, color: const Color(0xFF216A44), size: isVertical ? 24 : 28),
        label: Text('Reservas', style: TextStyle(color: const Color(0xFF216A44), fontSize: sizeFont, fontWeight: FontWeight.w900)),
      ),
      TextButton.icon(
        onPressed: () {
        ...
        label: Text('Perfil', style: TextStyle(color: const Color(0xFF216A44), fontSize: sizeFont)),
      ),
    ];
  }
  ...
  Widget build(BuildContext context) {
    ...
    final bool esDesktop = size.width > 950;
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        leading: esDesktop 
            ...
            : null, // Muestra el botón de hamburguesa automáticamente si no es desktop
        title: esDesktop
            ? SizedBox(
                width: 400,
                child: SearchBar(
            ...
            : const Text('Reservas', style: TextStyle(color: Colors.black, fontWeight: FontWeight.bold)),
        actions: [
          Padding(
            padding: EdgeInsets.only(right: esDesktop ? 20.0 : 10.0),
            child: Tooltip(
              message: 'Cerrar sesión', preferBelow: true, verticalOffset: 25,
                  ...
                  UserAccountsDrawerHeader(
                    ...
                    currentAccountPicture: CircleAvatar(
                      ...
                      backgroundImage: (widget.prestador.imagenUrl != null && widget.prestador.imagenUrl!.isNotEmpty)
                          ...
                          : null,
                      child: (widget.prestador.imagenUrl == null || widget.prestador.imagenUrl!.isEmpty)
                          ? const Icon(Icons.person, size: 40, color: Color(0xFF216A44))
                          : null,
                    ),
                  ),
              ...
              Center(
                child: Padding(
                  padding: EdgeInsets.symmetric(
                    horizontal: esDesktop ? 0.0 : 16.0,
                    vertical: 30.0,
                  ),
                  child: Container(
                    width: 1240,
                    // Dejamos que crezca dinámicamente si no es desktop para albergar la lista de cards
                    height: esDesktop ? 560 : null, 
                    decoration: BoxDecoration(
                      color: const Color(0xFFFFFFFF), 
                      borderRadius: BorderRadius.circular(25),
                    ), 
                    child: Padding(
                      padding: EdgeInsets.all(esDesktop ? 30.0 : 16.0),
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        mainAxisSize: MainAxisSize.min,
                        children: [
                                ...
                                return GestureDetector(
                                  ...
                                  child: Container(
                                    ...
                                    decoration: BoxDecoration(
                                      ...
                                      boxShadow: esActivo ? [BoxShadow(color: Colors.black.withOpacity(0.05), blurRadius: 4)] : [],
                                    ),
                                    child: Text(
                                      filtro,
                                      style: TextStyle(
                          ...
                          // Manejo de altura adaptativo para el FutureBuilder
                          SizedBox(
                            height: esDesktop ? 350 : null,
                            child: FutureBuilder<List<ReservaUIWrapper>>(
                              ...
                              builder: (context, snapshot) {
                                ...
                                if (!snapshot.hasData || snapshot.data!.isEmpty) {
                                  return const Center(child: Padding(
                                    padding: EdgeInsets.all(40.0),
                                    child: Text('No has recibido ninguna reserva todavía.', style: TextStyle(fontSize: 22, color: Colors.grey)),
                                  ));
                                }
                                ...
                                if (reservasFiltradas.isEmpty) {
                                  return const Center(child: Padding(
                                    padding: EdgeInsets.all(40.0),
                                    child: Text('No hay reservas en esta categoría.', style: TextStyle(fontSize: 18, color: Colors.grey)),
                                  ));
                                }

                                // SI ES MÓVIL: Renderizar en formato de Lista de Tarjetas (Cards)
                                if (!esDesktop) {
                                  return ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      return _buildFilaReservaMobile(reservasFiltradas[index]);
                                    },
                                  );
                                }

                                // SI ES DESKTOP: Renderizar en formato Tabla original
                                return Container(
                                  decoration: BoxDecoration(
                                  ...
                                  child: Column(
                                    children: [
                                      Container(
                                        ...
                                        child: Row(
                                          children: const [
                                            Expanded(flex: 2, child: Text('Viajero', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF526F75)))),
                                            Expanded(flex: 2, child: Text('Destino', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF526F75)))),
                                            Expanded(flex: 2, child: Text('Estadía (In - Fin)', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF526F75)))),
                                            Expanded(flex: 1, child: Text('Cupos', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF526F75)))),
                                            Expanded(flex: 1, child: Text('Total', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF526F75)))),
                                            Expanded(flex: 2, child: Text('Estado', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF526F75)))),
                                            Expanded(flex: 2, child: Text('Acciones', style: TextStyle(fontWeight: FontWeight.bold, color: Color(0xFF526F75)))),
                                          ],
                                        ),
                                      ),
                                      Expanded(
  ...
  Widget _buildFilaReservaWeb(ReservaUIWrapper item) {
    ...
    return Container(
      ...
      child: Row(
        children: [
          Expanded(flex: 2, child: Text(item.nombreViajero, style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15))),
          Expanded(flex: 2, child: Text(item.tituloPublicacion, style: const TextStyle(color: Colors.black87, fontSize: 15))),
          ...
          Expanded(flex: 1, child: Text('${reserva.total.toInt()}\$', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15))),
          Expanded(flex: 2,
            child: Align(alignment: Alignment.centerLeft,
              child: Container(
                padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 4),
                decoration: BoxDecoration(color: statusBg, borderRadius: BorderRadius.circular(12)),
                child: Text(etiquetaEstado, style: TextStyle(color: statusText, fontWeight: FontWeight.w600, fontSize: 13)),
              ),
            ),
          ),
          Expanded(flex: 2,
  ...
  Widget _buildFilaReservaMobile(ReservaUIWrapper item) {
    final reserva = item.reserva;
      ...
      child: Column(
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(color: statusBg, borderRadius: BorderRadius.circular(12)),
                child: Text(etiquetaEstado, style: TextStyle(color: statusText, fontWeight: FontWeight.bold, fontSize: 12)),
              ),
            ],
          ),
          const SizedBox(height: 8),
          Text(
            'Destino: ${item.tituloPublicacion}',
          ...
          Row(
            ...
            children: [
              Text(
                'Cupos: ${reserva.cupos} | Total: ${reserva.total.toInt()}\$',
                ...
                Row(
                  ...
                  children: [
                    IconButton(
                      ...
                      icon: const Icon(Icons.check_circle, color: Color(0xFF387653), size: 28),
                      onPressed: () async {
                        if (_verificarSuspensionYAlertar()) return;
                        await _gestionReservacion.confirmarReserva(reserva.id);
                        _cargarReservas();
                    ...
                    IconButton(
                      padding: EdgeInsets.zero,
                      constraints: const BoxConstraints(),
                      icon: const Icon(Icons.cancel, color: Color(0xFF8A1C14), size: 28),
                      onPressed: () async {
                        if (_verificarSuspensionYAlertar()) return;
                        await _gestionReservacion.cancelarReserva(reserva.id);
                        _cargarReservas();
